...
#include "mdaEPiano.h"
#include "mdaEPianoInstruments.h"
...
mdaEPiano::mdaEPiano(uint8_t nvoices) // mdaEPiano::mdaEPiano(audioMasterCallback audioMaster) : AudioEffectX(audioMaster, NPROGS, NPARAMS)
{
 ...
 // EEPROM.begin(2048);	// Start 2k eeprom emulation
  ...
  //mda-Defaults: 33 Keygroups (0..32), 3 Velocity-Layer je Region
  nKeygroups = 33;
  noteonStep  = 3;
  defaultNKeygroups = 33;
  for(int32_t i=0;i<33;i++) defaultKgrp[i] = kgrp[i];
  currentInstr = 0;
  strncpy(instrName, "Rd I", sizeof(instrName)-1);
  instrName[sizeof(instrName)-1] = 0;

  //HINWEIS: Die XFade-Loop-Ueberblendung ist mittlerweile in epianoData
  //VOR-eingerechnet (pre-baked, siehe include/mdaEPianoData.h). Eine
  //Laufzeit-XFade-Schleife entfaellt -> epianoData bleibt const/nur-lesen und
...
void mdaEPiano::loadInstrument(const int16_t* data, const KGRP* kgrpTable,
                               int32_t nkg, int32_t step)
{
  if(nkg <= 0 || nkg > (int32_t)(sizeof(kgrp)/sizeof(kgrp[0]))) return;  // Schutz
  waves       = (short*)data;
  noteonStep  = (step > 0) ? step : 3;
  nKeygroups = nkg;
  for(int32_t i = 0; i < nkg; i++)
  {
    kgrp[i].root = kgrpTable[i].root;
    kgrp[i].high = kgrpTable[i].high;
    kgrp[i].pos  = kgrpTable[i].pos;
    kgrp[i].end  = kgrpTable[i].end;
    kgrp[i].loop = kgrpTable[i].loop;
  }
  resetVoices();   // alte Stimmen (mit alten pos/end) beenden
}


void mdaEPiano::setInstrument(int32_t idx)
{
  if(idx < 0 || idx >= MDA_NINSTR) return;
  if(idx == 0) {                       // mda-Default (epianoData, XFade pre-baked in include/mdaEPianoData.h)
    waves = (short*)epianoData;
    for(int32_t i=0;i<defaultNKeygroups;i++) kgrp[i] = defaultKgrp[i];
    nKeygroups = defaultNKeygroups;
    noteonStep = 3;
    resetVoices();
  } else {                             // neues Instrument laden (Daten nur gelesen)
    const MdaInstrument& in = mdaInstruments[idx];
    loadInstrument(in.data, in.kgrp, in.nKeygroups, in.noteonStep);  // ruft resetVoices()
  }
  currentInstr = idx;
  strncpy(instrName, mdaInstruments[idx].name, sizeof(instrName)-1);
  instrName[sizeof(instrName)-1] = 0;
}
